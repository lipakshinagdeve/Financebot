...
class MockAIService:
    ...
    def _generate_expense_response(self, amount, category, current_spending, budget, total_spent, income, current_savings, savings_goal):
        remaining_budget = budget - current_spending
        remaining_income = income - total_spent
        savings_needed = savings_goal - current_savings
        
        # Budget status for this category
        if budget == 0:
            budget_status = "You haven't set a budget for this category yet."
        elif remaining_budget < 0:
            over_amount = abs(remaining_budget)
            budget_status = f"You're ${over_amount:.2f} over your {category} budget!"
        else:
            pct_remaining = calculate_percentage(remaining_budget, budget)
            budget_status = f"You have ${remaining_budget:.2f} ({pct_remaining:.0f}%) left in your {category} budget."
        
        # Overall spending status
        spending_pct = calculate_percentage(total_spent, income)
        if spending_pct > 90:
            overall_status = f"⚠️ You've used {spending_pct:.0f}% of your monthly income."
        elif spending_pct > 75:
            overall_status = f"You've spent {spending_pct:.0f}% of your income this month."
        else:
            overall_status = f"You're doing well - {spending_pct:.0f}% of income spent."
        
        # Savings goal status
        if savings_needed > 0:
            savings_pct = calculate_percentage(current_savings, savings_goal)
            savings_status = f"💰 You're {savings_pct:.0f}% toward your ${savings_goal:,.0f} savings goal (${savings_needed:,.0f} to go)."
        else:
            savings_status = "🎉 You've reached your savings goal!"

        response = (
            f"✅ Recorded ${amount:.2f} for {category}.\n\n"
            f"📊 **Category Status:** {budget_status}\n\n"
            f"💵 **Monthly Overview:** {overall_status}\n\n"
            f"{savings_status}"
        )
        return response
    ...
    def chat(self, user_input, expenses, budgets, income, spending_goal, savings_goal, chat_history):
        ...
        if amount and category:
            ...
            total_spent = sum(e['amount'] for e in expenses) + amount
            budget = budgets.get(category, 0.0)
            
            # Get savings info
            current_savings = 3200.00  # This should come from session state
            
            response_text = self._generate_expense_response(
                amount, category, new_category_spending, budget,
                total_spent, income, current_savings, savings_goal
            )
            
            return response_text, new_expense 
            
        else:
            return "I can help you track expenses! Try saying something like 'I spent $20 on coffee' or 'paid $50 for groceries'.", None
...
with col_input:
    user_input = st.text_input("Message", placeholder="Type your message...", label_visibility="collapsed", key="chat_input", on_change=lambda: st.session_state.update({'submit_triggered': True}))
...
if user_input and (send or st.session_state.get('submit_triggered', False)):
    # Reset trigger
